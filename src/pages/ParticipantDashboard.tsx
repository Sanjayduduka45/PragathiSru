          ...
          <div className="relative z-10 w-full max-w-5xl bg-slate-100 rounded-3xl shadow-2xl border border-slate-200 flex flex-col h-[92vh] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center" ref={containerRef}>
              ...
              <div
                ...
                className="bg-white border-[24px] border-[#004182] p-8 flex flex-col justify-between shadow-2xl shrink-0 select-text relative"
              >
                ...
                {/* Project Title Block (Editable Title, Members, Dept) */}
                <div className="bg-[#004182] text-white rounded-2xl p-5 mb-5 text-center shadow-md border border-[#003166]">
                  ...
                  <div
                    ...
                    onBlur={(e) => setFields((prev) => ({ ...prev, projectTitle: e.target.innerText }))}
                    ...
                    className={`outline-hidden text-2xl font-black tracking-tight mb-2 uppercase min-h-[36px] ${
                      !isSubmitted ? 'focus:ring-2 focus:ring-blue-400 focus:bg-blue-950 rounded px-1' : ''
                    }`}
                  >
                    {fields.projectTitle || profile.projectTitle || 'PROJECT TITLE'}
                  </div>
                  ...
                  <div className="text-sm font-bold text-blue-100 tracking-wide mb-1 flex items-center justify-center gap-1.5">
                    ...
                    <div
                      contentEditable={!isSubmitted}
                      suppressContentEditableWarning
                      onBlur={(e) => setFields((prev) => ({ ...prev, teamMembers: e.target.innerText }))}
                      className={`outline-hidden min-w-[150px] ${
                        !isSubmitted ? 'focus:ring-2 focus:ring-blue-400 focus:bg-blue-950 rounded px-1' : ''
                      }`}
                    >
                      {fields.teamMembers || defaultMembers}
                    </div>
                  </div>
                  ...
                  <div
                    contentEditable={!isSubmitted}
                    suppressContentEditableWarning
                    onBlur={(e) => setFields((prev) => ({ ...prev, departmentDetails: e.target.innerText }))}
                    className={`outline-hidden text-xs font-semibold text-slate-300 uppercase tracking-wider min-h-[16px] ${
                      !isSubmitted ? 'focus:ring-2 focus:ring-blue-400 focus:bg-blue-950 rounded px-1' : ''
                    }`}
                  >
                    {fields.departmentDetails || defaultDept}
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <div className="border-2 border-indigo-900/10 rounded-2xl p-5 bg-slate-50/50 flex-1 flex flex-col min-h-0 shadow-2xs">
                      ...
                      <div
                        contentEditable={!isSubmitted}
                        suppressContentEditableWarning
                        onBlur={(e) => setFields((prev) => ({ ...prev, introduction: e.target.innerText }))}
                        className={`outline-hidden text-xs text-slate-700 leading-relaxed overflow-y-auto whitespace-pre-wrap flex-1 text-left ${
                          !isSubmitted ? 'focus:ring-2 focus:ring-indigo-500 rounded p-1 focus:bg-white' : ''
                        }`}
                        placeholder="Click to type project introduction and background details here…"
                      >
                        {fields.introduction || ''}
                      </div>
                    </div>
                    ...
                    <div className="border-2 border-indigo-900/10 rounded-2xl p-5 bg-slate-50/50 flex-1 flex flex-col min-h-0 shadow-2xs">
                      ...
                      <div
                        contentEditable={!isSubmitted}
                        suppressContentEditableWarning
                        onBlur={(e) => setFields((prev) => ({ ...prev, methodology: e.target.innerText }))}
                        className={`outline-hidden text-xs text-slate-700 leading-relaxed overflow-y-auto whitespace-pre-wrap flex-1 text-left ${
                          !isSubmitted ? 'focus:ring-2 focus:ring-indigo-500 rounded p-1 focus:bg-white' : ''
                        }`}
                        placeholder="Describe research objectives, methods, and system block diagram flow here…"
                      >
                        {fields.methodology || ''}
                      </div>
                    </div>
                      ...
                      <div className="flex-1 grid grid-cols-3 gap-3 min-h-0 items-center justify-center">
                        ...
                        <div className="flex flex-col items-center">
                          <div className="relative w-full aspect-square border border-dashed border-slate-300 rounded-xl bg-white flex items-center justify-center overflow-hidden group">
                            {fields.diagram1 ? (
                              <img src={fields.diagram1} className="w-full h-full object-contain" alt="Fig 1" />
                            ) : (
                            ...
                            {!isSubmitted && (
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (file) handleImageUpload('diagram1', file);
                                }}
                          ...
                          <div
                            contentEditable={!isSubmitted}
                            suppressContentEditableWarning
                            onBlur={(e) => setFields((prev) => ({ ...prev, diagram1Caption: e.target.innerText }))}
                            className={`outline-hidden text-[9px] font-bold text-slate-500 mt-1.5 text-center leading-tight min-h-[12px] min-w-[70px] ${
                              !isSubmitted ? 'focus:ring-1 focus:ring-indigo-500 rounded px-0.5' : ''
                            }`}
                          >
                            {fields.diagram1Caption || 'Fig. 1. Prototype config'}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                          <div className="relative w-full aspect-square border border-dashed border-slate-300 rounded-xl bg-white flex items-center justify-center overflow-hidden group">
                            {fields.diagram2 ? (
                              <img src={fields.diagram2} className="w-full h-full object-contain" alt="Fig 2" />
                            ) : (
                              <div className="text-center p-2 text-slate-400">
                                <Upload className="w-5 h-5 mx-auto mb-1 opacity-60" />
                            ...
                            {!isSubmitted && (
                              <input
                                ...
                                onChange={(e) => {
                                  const file = e.target.files?.[0];
                                  if (file) handleImageUpload('diagram2', file);
                                }}
                                className="absolute inset-0 opacity-0 cursor-pointer"
                              />
                            )}
                          ...
                          <div
                            contentEditable={!isSubmitted}
                            suppressContentEditableWarning
                            onBlur={(e) => setFields((prev) => ({ ...prev, diagram2Caption: e.target.innerText }))}
                            className={`outline-hidden text-[9px] font-bold text-slate-500 mt-1.5 text-center leading-tight min-h-[12px] min-w-[70px] ${
                              !isSubmitted ? 'focus:ring-1 focus:ring-indigo-500 rounded px-0.5' : ''
                            }`}
                          >
                            {fields.diagram2Caption || 'Fig. 2. System design'}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                          <div className="relative w-full aspect-square border border-dashed border-slate-300 rounded-xl bg-white flex items-center justify-center overflow-hidden group">
                            {fields.diagram3 ? (
                              <img src={fields.diagram3} className="w-full h-full object-contain" alt="Fig 3" />
                            ) : (
                              <div className="text-center p-2 text-slate-400">
                                <Upload className="w-5 h-5 mx-auto mb-1 opacity-60" />
                            ...
                            {!isSubmitted && (
                              <input
                                ...
                                onChange={(e) => {
                                  const file = e.target.files?.[0];
                                  if (file) handleImageUpload('diagram3', file);
                                }}
                                className="absolute inset-0 opacity-0 cursor-pointer"
                              />
                            )}
                          ...
                          <div
                            contentEditable={!isSubmitted}
                            suppressContentEditableWarning
                            onBlur={(e) => setFields((prev) => ({ ...prev, diagram3Caption: e.target.innerText }))}
                            className={`outline-hidden text-[9px] font-bold text-slate-500 mt-1.5 text-center leading-tight min-h-[12px] min-w-[70px] ${
                              !isSubmitted ? 'focus:ring-1 focus:ring-indigo-500 rounded px-0.5' : ''
                            }`}
                          >
                            {fields.diagram3Caption || 'Fig. 3. Result responses'}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-2 border-indigo-900/10 rounded-2xl p-5 bg-slate-50/50 flex-1 flex flex-col gap-4 min-h-0 shadow-2xs">
                      ...
                      <div className="flex flex-col min-h-0">
                        ...
                        <div
                          contentEditable={!isSubmitted}
                          suppressContentEditableWarning
                          onBlur={(e) => setFields((prev) => ({ ...prev, conclusion: e.target.innerText }))}
                          className={`outline-hidden text-xs text-slate-700 leading-relaxed overflow-y-auto whitespace-pre-wrap text-left ${
                            !isSubmitted ? 'focus:ring-2 focus:ring-indigo-500 rounded p-1 focus:bg-white' : ''
                          }`}
                          placeholder="Summarise outcomes and conclusions of your project…"
                        >
                          {fields.conclusion || ''}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col min-h-0 border-t border-dashed border-slate-200 pt-2">
                        ...
                        <div
                          contentEditable={!isSubmitted}
                          suppressContentEditableWarning
                          onBlur={(e) => setFields((prev) => ({ ...prev, references: e.target.innerText }))}
                          ...
                          placeholder="1. Author, Title, Journal Name, Date…"
                        >
                          {fields.references || ''}
                        </div>
                      </div>
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-xs shadow-slate-900/5">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0 group focus:outline-none" aria-label="Back to PRAGATHI 2K26">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-[#004182] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-200">
              <img src={pragathiLogo} alt="PRAGATHI" className="h-6 sm:h-7 w-auto object-contain" />
            </div>
